...
def inception_network():
    ...
    * Returns: the keras model
    ...
    conv_1 = K.layers.Conv2D(filters=64, kernel_size=(7, 7), strides=(2, 2), padding='same',
                            activation=activation, kernel_initializer=init)(Y)
    max_pool1 = K.layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2),
                                     padding='same')(conv_1)
    conv_2R = K.layers.Conv2D(filters=64, kernel_size=(1, 1), strides=(1, 1), padding='same',
                             activation=activation, kernel_initializer=init)(max_pool1)
    conv_2 = K.layers.Conv2D(filters=192, kernel_size=(3, 3), strides=(1, 1), padding='same',
                            activation=activation, kernel_initializer=init)(conv_2R)
